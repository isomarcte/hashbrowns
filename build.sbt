...
def rubySetupSteps(cond: Option[String]) = Seq(
  WorkflowStep.Use(
    ref = UseRef.Public("ruby", "setup-ruby", "v1"),
...
ThisBuild / githubWorkflowPublishPreamble ++=
  WorkflowStep.Use(ref = UseRef.Public("olafurpg", "setup-gpg", "v3")) +: rubySetupSteps(None)
...
lazy val site = project.in(file("site"))
  ...
  .settings{
    import microsites._
    Seq(
      micrositeName := "hashbrowns",
      micrositeDescription := "Hashing Tools",
      micrositeAuthor := "Christopher Davenport",
      micrositeGithubOwner := "ChristopherDavenport",
      micrositeGithubRepo := "hashbrowns",
      micrositeBaseUrl := "/hashbrowns",
      micrositeDocumentationUrl := "https://www.javadoc.io/doc/io.chrisdavenport/hashbrowns_2.13",
...
inThisBuild(List(
  ...
  homepage := Some(url("https://github.com/ChristopherDavenport/hashbrowns")),
  ...
  Compile / doc / scalacOptions ++= Seq(
      ...
      "-doc-source-url", "https://github.com/ChristopherDavenport/hashbrowns/blob/v" + version.value + "€{FILE_PATH}.scala"
  )
))
